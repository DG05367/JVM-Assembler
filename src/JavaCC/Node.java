...
public interface Node extends Comparable<Node>  {
    ...
    /**
      * @return the #JasminParserLexer from which this Node object
      ...
      * machinery.
      */
    JasminParserLexer getTokenSource();
    void setTokenSource(JasminParserLexer tokenSource);
    /**
      * @return the original source content this Node came from
      * a reference to the #JasminParserLexer that stores the source code and
    ...
    default Token firstDescendantOfType(JasminParserConstants.TokenType type) {
        for (int i= 0; i<getChildCount(); i++) {
            Node child= getChild(i);
            if (child instanceof Token) {
                Token tok= (Token) child;
    ...
    default Token firstChildOfType(JasminParserConstants.TokenType tokenType) {
        for (int i= 0; i<getChildCount(); i++) {
            Node child= getChild(i);
            if (child instanceof Token) {
                Token tok= (Token) child;
    ...
    default JasminParserConstants.TokenType getTokenType() {
    ...
    default void dump(String prefix) {
        String output= (this instanceof Token)?toString().trim():
